...
export default function LawyerSignInPage() {
  ...
  const verification = useEmailVerificationUrl("lawyer")
  ...
  useEffect(() => {
    if (verification.status === "verified") {
      const msg =
        "Email verified successfully! You can now sign in. Your bar license may still be pending admin review."
  ...
  const handleResendVerification = async () => {
    ...
    try {
      const res = await fetch("/api/auth/send-verification-email", {
        ...
        body: JSON.stringify({ email: targetEmail, userType: "lawyer" }),
      })
      if (!res.ok) {
        ...
        <form onSubmit={handleSignIn} className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Link href="/auth/forgot-password?from=lawyer" className="text-sm text-blue-600 hover:underline">
                Forgot password?
              </Link>
            </div>
            <Input
